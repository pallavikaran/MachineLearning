...
def kappa(y_true, y_pred, weights=None, allow_off_by_one=True):
    '''
...
for i in range(2, 591):
    actual_score = ws.cell(row = i, column = 5).value
    predict_score = ws.cell(row = i, column = 6).value
    a.append(actual_score)
    b.append(predict_score)

print(kappa(a,b,weights=None,allow_off_by_one=True))
...
for i in range(2, 591):
	actual_score = ws.cell(row = i, column = 5).value
	predict_score = ws.cell(row = i, column = 6).value
	if ((actual_score - predict_score) <= 1):
		right = right + 1
	else:
		wrong = wrong + 1
